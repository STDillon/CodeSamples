...
#TODO: fix these imports if the need arises
    ...
    def handle_twitter_http_error(e, wait_period=2, sleep_when_rate_limited=True):
        ...
        if e.e.code == 401:
            ...
            if sleep_when_rate_limited:
                print("Retrying in 15 due to twitter's API constraints")
                sys.stderr.flush()
                time.sleep(60 * 15 + 5)
                print('Alright, it\'s go time once again, ready to retry.')
                return 2
            else:
                raise e  # Caller must handle the rate limiting issue
        elif e.e.code in (500, 502, 503, 504):
            print('Encountered {0} Error. Retrying in {1} seconds'.format(e.e.code, wait_period))
